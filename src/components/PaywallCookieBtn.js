...
class PaywallCookieBtn extends Component {
  constructor(props) {
    ...
    this.state = {
      inCookieWhitelist:
        chrome.extension.getBackgroundPage().paywallInCookieWhitelist
    };
  }
  ...
  render() {
    ...
    if (bg.paywallEnabled) {
      if (this.state.inCookieWhitelist)
        listBtn = (
          <Button
            onClick={this.removeFromCookieWhitelist}
            variant='warning'
            style={{ fontSize: '15px' }}
          >
            Block Cookies
        ...
        listBtn = (
          <Button
            onClick={this.addToCookieWhitelist}
      ...
      listBtn = (
        <Button variant='outline-info' style={{ fontSize: '15px' }} disabled>
